...
func getDataFromDatabase(recordbook : Int)
{
    
    var request = URLRequest(url: URL(string: "http://185.58.205.8/GetInfo.php")!)
    request.httpMethod = "POST"
    var bodyData = "recordbook=\(recordbook)"
    ...
    let task = URLSession.shared.dataTask(with: request) { data, response, error in
        ...
        do {
            ...
            if let convertedJsonIntoDict = try JSONSerialization.jsonObject(with: data, options: []) as? NSArray {
                ...
                let name = (convertedJsonIntoDict[0] as! NSDictionary)["Name"] as? String
                let surname = (convertedJsonIntoDict[0] as! NSDictionary)["Surname"] as? String
                let middlename = (convertedJsonIntoDict[0] as! NSDictionary)["MiddleName"] as? String
                let group = (convertedJsonIntoDict[0] as! NSDictionary)["GroupName"] as? String
                let recordbook = (convertedJsonIntoDict[0] as! NSDictionary)["RecordBook"] as? String
                
                
                //var student = Student(id:Int(id!)!,Name: name!,Surname: surname!,MiddleName: middlename!,Group: group!,RecordBook: Int(recordbook!)!)
